...
tree$tip.label <- paste(toupper(substring(tree$tip.label, 1, 1)), substring(tree$tip.label, 2), sep = "") # 学名表示
trait <- read.csv("/Users/uri/Dropbox/git/Data-analyses/life-history_traits_and_stool_structure/Data/KNUT_observation_summary.csv", header = TRUE) %>% 
  arrange(Phylo.Order) %>% 
  select(Abbreviation, "Sprout ratio" = rs.ratio, "Number of sprout" = num.rs, SSI = AUC.Value, "Sprout RBA" = ba.size.ratio.old)
...
shade.cat <- table(rank.mean.h, df$Overhead) # 各種のCPIをカテゴリーで分ける
shade.cat <- t(shade.cat) # 行列の入れ替え
stem.n <- apply(shade.cat, 2, sum)
cat1 <- shade.cat[1, ] / stem.n * 100
cat2 <- shade.cat[2, ] / stem.n * 100
cat3 <- shade.cat[3, ] / stem.n * 100
df.shade <- rbind(cat1, cat2, cat3)

df.shade <- as.data.frame(t(df.shade)) %>% gather(key = shade.cat)
...
ggplot(df.shade, aes(x = Abb2, y = value, fill = shade.cat)) +
...
ggplot(data = df, aes(x = AUC.Value, y = shade.c1)) +
  ...
  scale_x_continuous("SSI", breaks = seq(0.5, 1.0, 0.25)) + ylab("Sprout shading") +
  theme(text = element_text(family = "Helvetica", size = 40),
...
ggplot(data = df.pic, aes(x = x, y = y)) +
  ...
  xlab("SSI(contrasts)") + ylab("Sprout shading(contrasts)") +
...
p1 <- ggplot(data = df, aes(x = rs.ratio, y = Height)) +
  ...
  scale_x_continuous(name = "Sprout ratio", breaks = seq(0, 100, 50)) + scale_y_continuous(name = "", limits = c(0, 25), breaks = seq(0, 25, 10)) +
    theme(text = element_text(family = "Helvetica", size = 46),
#         axis.title = element_text(size = 46),
...
p2 <- ggplot(data = df, aes(x = num.rs, y = Height)) +
  ...
  annotate("text", x = max(df$num.rs) * 0.7, y = 24,
           ...
           label = label.r,
           parse = TRUE, size = 12, family = "Helvetica") +
  scale_x_continuous(name = "Number of sprout", breaks = c(0, 4, 8)) + scale_y_continuous(name = "", limits = c(0, 25), breaks = seq(0, 25, 10)) +
    theme(text = element_text(family = "Helvetica", size = 46),
#         axis.title = element_text(size = 46),
        axis.text.x = element_text(family = "Helvetica", size = 32),
        axis.text.y = element_text(family = "Helvetica", size = 32),
...
p3 <- ggplot(data = df, aes(x = AUC.Value, y = Height)) +
  ...
  annotate("text", x = 0.85, y = 24,
           ...
           label = label.r,
           parse = TRUE, size = 12, family = "Helvetica") +
  scale_x_continuous(name = "SSI", breaks = c(0.5, 0.75, 1.0)) + scale_y_continuous(name = "", limits = c(0, 25), breaks = seq(0, 25, 10)) +
      theme(text = element_text(family = "Helvetica", size = 46),
#         axis.title = element_text(size = 46),
        axis.text.x = element_text(family = "Helvetica", size = 32),
        axis.text.y = element_text(family = "Helvetica", size = 32),
...
p4 <- ggplot(data = df, aes(x = ba.size.ratio.old, y = Height)) +
  ...
  annotate("text", x = max(df$ba.size.ratio.old) * 0.7, y = 24,
           ...
           label = c(label.r),
           parse = TRUE, size = 12, family = "Helvetica") +
  scale_x_continuous(name = "Sprout RBA", breaks = c(0, 0.3, 0.6)) + scale_y_continuous(name = "", limits = c(0, 25), breaks = seq(0, 25, 10)) +
      theme(text = element_text(family = "Helvetica", size = 46),
#         axis.title = element_text(size = 46),
        axis.text.x = element_text(family = "Helvetica", size = 32),
        axis.text.y = element_text(family = "Helvetica", size = 32),
...
grid.arrange(p1, p2, p3, p4, 
             ncol = 2, 
             left = textGrob("Height (m)", rot = 90, vjust = 1, gp = gpar(fontsize = 46)))
```
